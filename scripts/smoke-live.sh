...
BACKEND_URL="${BACKEND_URL:-https://uptimepulse-backend-mee4.onrender.com}"
FRONTEND_URL="${FRONTEND_URL:-https://uptimepulse-frontend.onrender.com}"
...
curl --fail --silent --show-error --request POST "${API_BASE_URL}/auth/login" \
  ...
  --data '{"email":"demo@uptimepulse.dev","password":"demo123"}' \
  | grep -q '"token"'
