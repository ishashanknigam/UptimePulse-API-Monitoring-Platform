  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      axios.get(`${API_BASE_URL}/status/${slug}`)
        ...
        .catch(() => { if (mounted) setError('Status page not found') })
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-3xl mx-auto px-6 py-12">
              ...
              {data.monitors.map(m => (
                <div key={m.id} className="flex items-center justify-between py-2 border-b border-slate-800 last:border-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={!m.active ? 'badge-yellow' : m.hasOpenIncident ? 'badge-red' : 'badge-green'}>
                      {!m.active ? 'Paused' : m.hasOpenIncident ? 'Down' : 'Operational'}
                    </span>
                  </div>
                </div>
              ))}
        ...
        <p className="text-center text-xs text-slate-600 mt-8">
          Powered by <a href="/" className="text-brand-500 hover:text-brand-400">UptimePulse</a>
        </p>
      </div>
    </div>
  )
