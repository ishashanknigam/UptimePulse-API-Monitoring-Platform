...
public class MonitoringMetricsService {
    ...
    public MonitoringMetricsService(MeterRegistry meterRegistry) {
        this.checksTotal = Counter.builder("uptimepulse_monitor_checks_total")
                .description("Total monitor checks processed by workers")
                .register(meterRegistry);
        this.checksSucceeded = Counter.builder("uptimepulse_monitor_checks_success_total")
                .description("Successful monitor checks")
                .register(meterRegistry);
        this.checksFailed = Counter.builder("uptimepulse_monitor_checks_failure_total")
                .description("Failed monitor checks")
                .register(meterRegistry);
        this.enqueueFailures = Counter.builder("uptimepulse_monitor_enqueue_failures_total")
                .description("Failures while enqueueing monitor checks")
                .register(meterRegistry);
        this.checkLatency = Timer.builder("uptimepulse_monitor_check_latency")
                .description("Observed latency for outbound monitor checks")
                .publishPercentileHistogram()
                .register(meterRegistry);

        Gauge.builder("uptimepulse_monitor_dispatch_batch_size", lastDispatchSize, AtomicInteger::get)
