package dev.uptimepulse;

import dev.uptimepulse.entity.*;
import dev.uptimepulse.entity.enums.*;
import dev.uptimepulse.repository.*;
...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (userRepository.existsByEmail("demo@uptimepulse.dev")) {
        ...
        alertRepository.save(Alert.builder()
                ...
                .status(AlertStatus.SENT)
                .message("[MOCK EMAIL] To: demo@uptimepulse.dev | Monitor 'API Health' has RECOVERED")
                .sentAt(Instant.now().minus(4, ChronoUnit.HOURS))
                .build());

        log.info("Demo data seeded. Login: demo@uptimepulse.dev / demo123");
    }
}
